...
namespace packt.FoodyGO.Services
{
    public class MonsterService : MonoBehaviour
    {
        ...
        public Monster getMonsterDetails(MonsterSpawnLocation monster)
        {
            int x = monsters.IndexOf(monster);

            Monster monsterDetail = monsters[x].monster;
        ...
        {            
            ...
            monster.gameObject = (GameObject)Instantiate(monstersPrefab[monster.monster.PrefabID], position, rotation);
            if (monster.monster.PrefabID <= 3)
            {
                monster.gameObject.transform.localScale = new Vector3(2, 2, 2);
            }
            else
            {
                monster.gameObject.transform.localScale = new Vector3(1, 1, 1);
            }
            var controller = monster.gameObject.AddComponent<MonsterController>(); //Need this!!
            //controller.currentMonster = monster.monster;
        ...
        {
            int i = Random.Range(0, monstersPrefab.Count);

            GameObject spawnThisMonster = monstersPrefab[i];

            return spawnThisMonster;
        }

        private GameObject SpawnRandomMonster()
        {
            var monster = MonsterFactory.CreateRandomMonster();

            GameObject spawnThisMonster = monstersPrefab[monster.PrefabID];

            return spawnThisMonster;
        }

        private Monster SpawnRandomMonster1()
        {
            var monster = MonsterFactory.CreateRandomMonster();
